...
#elif defined(SUPPORT_GPL_UNDEFINED)
#if defined(SUPPORT_GPL_UNDEFINED)
...
#if defined(SUPPORT_GPL_UNDEFINED) || defined(AEI_VDSL_CUSTOMER_SASKTEL_STATIC_MAC_ADDR)
...
int kerSysGetMacAddresses( unsigned char *pucaMacAddr, unsigned int num_addresses, unsigned long ulId )
{
...
#if defined(SUPPORT_GPL_UNDEFINED)
    /*
     *  1. It is used to keep using WAN MAC address is same as before while upgrade XXX SDK3/SDK6 image to SDK12.
     ...
     *  5. For other XXX project (fox example T2200H), my suggestion is that using WAN interface(atm0/ptm0.1/ewan0.1) is B + 1, and atm1 = B + 2.
    ...
    if(ulId == 0x13ffffff)
    {
        ...
        if (aeiBoardId == AEI_BOARD_V2200H ||aeiBoardId == AEI_BOARD_T2200H )
        {
            if (i == 1)
                continue;
        }
        ...
        if (i == AEI_QTN_BSSID_INDEX)  /*This mac addr is used for Quantenna wifi0*/
        {
            continue;
        }
#endif
...
static PFILE_TAG getTagFromPartition(int imageNumber)
{
   ...
   switch( pTag->tagVersion[0] )
   {
       ...
       if (tagBuf == NULL)
       {
           if (NULL == (tagBuf = kmalloc(mtd->erasesize, GFP_KERNEL)))
               return ret;
       }
       ...
       {
         int blk_addr = 0;
         int total_tag_blks = AEI_TAG_BLOCKS * mtd->erasesize;
         pTag->tagVersion[0] = tag_not_found;

         for( blk_addr = 0; blk_addr < total_tag_blks && pTag->tagVersion[0] != tag_found; blk_addr += mtd->erasesize )
         {
           memset(tagBuf, 0, mtd->erasesize);
           mtd_read(mtd, blk_addr, mtd->erasesize, &retlen, tagBuf);
           memcpy(pTag, (unsigned char *)tagBuf, sizeof(FILE_TAG));

           if(strlen(pTag->signiture_1)>0 && strlen(pTag->signiture_1)<SIG_LEN)
           {
               pTag->tagVersion[0] = tag_found;
               ret = pTag;
           //    printk("#################block %d : %s\n",blk_addr/mtd->erasesize, pTag->signiture_1);
           }
         }
           ...
           pTag->tagVersion[0] = tag_not_found;
       /*get tag failed,try to get backup tag.backup tag is stored in tag of not booted partition*/
#if defined(AEI_VDSL_BACKUP_TAG)
       #define  BACKUP_TAG_OFFSET  512
       if(pTag->tagVersion[0] == tag_not_found)
       {
           if(rootfs_ofs == inMemNvramData.ulNandPartOfsKb[NP_ROOTFS_1])
           {
               if(imageNumber==1)
           ...
           if(mtd)
           {
               ...
               mtd_read(mtd, 0, mtd->erasesize, &retlen, tagBuf);
               memcpy(pTag, (unsigned char *)tagBuf + BACKUP_TAG_OFFSET, sizeof(FILE_TAG));
                       ...
                       unsigned int command, unsigned long arg )
{
         ...
         case BOARD_IOCTL_ADJUST_NVRAM:
            {
                ...
                {
                    NVRAM_DATA nvramData;
                    XXX_V2000H_NVRAM_DATA XXX_v2000h_nvramData;
                    AEI_readNvramData(&nvramData);
                    AEI_readNvramData((PNVRAM_DATA)&XXX_v2000h_nvramData);
                    if(nvramData.ulVersion == 6
						&& XXX_v2000h_nvramData.ulSerialNumber[0] != '\0'
						&& XXX_v2000h_nvramData.chFactoryFWVersion[0] != '\0'
						&& (unsigned char)XXX_v2000h_nvramData.ulSerialNumber[0] != 0xFF
						&& (unsigned char)XXX_v2000h_nvramData.chFactoryFWVersion[0] != 0xFF
						&& (nvramData.wlanParams[2] != 'p' || nvramData.wlanParams[3] != 'c' || nvramData.wlanParams[4] != 'i')
						&& strncmp(nvramData.ulSerialNumber,XXX_v2000h_nvramData.ulSerialNumber,32)!=0)
                    {
                        memset(&nvramData, 0, sizeof(nvramData));

                        memcpy(&nvramData.szBootline[0],&XXX_v2000h_nvramData.szBootline[0],NVRAM_BOOTLINE_LEN);
                        memcpy(&nvramData.szBoardId[0],&XXX_v2000h_nvramData.szBoardId[0],NVRAM_BOARD_ID_STRING_LEN);
                        nvramData.ulMainTpNum = XXX_v2000h_nvramData.ulMainTpNum;
                        nvramData.ulPsiSize = XXX_v2000h_nvramData.ulPsiSize;
                        nvramData.ulNumMacAddrs = XXX_v2000h_nvramData.ulNumMacAddrs;
                        memcpy(&nvramData.ucaBaseMacAddr[0],&XXX_v2000h_nvramData.ucaBaseMacAddr[0],NVRAM_MAC_ADDRESS_LEN);

                        nvramData.pad = XXX_v2000h_nvramData.pad;
                        nvramData.backupPsi = XXX_v2000h_nvramData.backupPsi;
                        nvramData.ulCheckSumV4 = XXX_v2000h_nvramData.ulCheckSumV4;

                        memcpy(&nvramData.gponSerialNumber[0],&XXX_v2000h_nvramData.gponSerialNumber[0],NVRAM_GPON_SERIAL_NUMBER_LEN);
                        memcpy(&nvramData.gponPassword[0],&XXX_v2000h_nvramData.gponPassword[0],NVRAM_GPON_PASSWORD_LEN);
                        memcpy(&nvramData.wpsDevicePin[0],&XXX_v2000h_nvramData.wpsDevicePin[0],NVRAM_WPS_DEVICE_PIN_LEN);
                        memcpy(&nvramData.wlanParams[0],&XXX_v2000h_nvramData.wlanParams[0],NVRAM_WLAN_PARAMS_LEN);

                        nvramData.ulSyslogSize = XXX_v2000h_nvramData.ulSyslogSize;

                        memcpy(&nvramData.ulNandPartOfsKb[0], &XXX_v2000h_nvramData.ulNandPartOfsKb[0], sizeof(nvramData.ulNandPartOfsKb));
                        memcpy(&nvramData.ulNandPartSizeKb[0], &XXX_v2000h_nvramData.ulNandPartSizeKb[0], sizeof(nvramData.ulNandPartSizeKb));
                        memcpy(&nvramData.szVoiceBoardId[0], &XXX_v2000h_nvramData.szVoiceBoardId[0], sizeof(nvramData.szVoiceBoardId));

                        memcpy(&nvramData.afeId[0], &XXX_v2000h_nvramData.afeId[0], sizeof(nvramData.afeId));

                        memcpy(&nvramData.ulSerialNumber[0],&XXX_v2000h_nvramData.ulSerialNumber[0],32);
                        memcpy(&nvramData.chFactoryFWVersion[0],&XXX_v2000h_nvramData.chFactoryFWVersion[0],48);
                        memcpy(&nvramData.wpsPin[0],&XXX_v2000h_nvramData.wpsPin[0],32);
                        memcpy(&nvramData.wpaKey[0],&XXX_v2000h_nvramData.wpaKey[0],32);
                        nvramData.dslDatapump=XXX_v2000h_nvramData.dslDatapump;

                        nvramData.ulVersion=NVRAM_VERSION_NUMBER;

						memset((char *)&nvramData + ((size_t) &((XXX_V2000H_NVRAM_DATA *)0)->ulSerialNumber), 0, 1);
						memset((char *)&nvramData + ((size_t) &((XXX_V2000H_NVRAM_DATA *)0)->chFactoryFWVersion), 0, 1);
...
#endif
#if defined(AEI_TWO_IN_ONE_FIRMWARE) && defined(SUPPORT_GPL_UNDEFINED)
 ...
 case  BOARD_IOCTL_GET_UG_INFO:
    ...
    case BOARD_IOCTL_GET_WLANLEDMODE:
        ...
        break;
#endif /*end SUPPORT_GPL_UNDEFINED*/
...
#if defined(SUPPORT_GPL) || defined(SUPPORT_GPL_UNDEFINED) || defined(SUPPORT_BACKUP_PSI)
...
#elif defined(SUPPORT_GPL_UNDEFINED)
/* XXX requirement:  If reset button is held up to 10 seconds, just reboot.  If held more than 10 seconds restore default
*/
static void AEI_resetBtnTimerFunc(unsigned long data)
{
#if defined(AEI_63168_CHIP)
...
static UBOOL8 AEI_DownGrade_AdjustNVRAM(void)
{
    ...
    if(nvramData.ulVersion == 6
         ...
         && strncmp(nvramData.ulSerialNumber,XXX_v2000h_nvramData.ulSerialNumber,32)!=0)
    {
        memset(&XXX_v2000h_nvramData, 0, sizeof(XXX_v2000h_nvramData));

        memcpy(&XXX_v2000h_nvramData.szBootline[0],&nvramData.szBootline[0],NVRAM_BOOTLINE_LEN);
        memcpy(&XXX_v2000h_nvramData.szBoardId[0],&nvramData.szBoardId[0],NVRAM_BOARD_ID_STRING_LEN);
        XXX_v2000h_nvramData.ulMainTpNum = nvramData.ulMainTpNum;
        XXX_v2000h_nvramData.ulPsiSize = nvramData.ulPsiSize;
        XXX_v2000h_nvramData.ulNumMacAddrs = nvramData.ulNumMacAddrs;
        memcpy(&XXX_v2000h_nvramData.ucaBaseMacAddr[0],&nvramData.ucaBaseMacAddr[0],NVRAM_MAC_ADDRESS_LEN);

        XXX_v2000h_nvramData.pad = nvramData.pad;
        XXX_v2000h_nvramData.backupPsi = nvramData.backupPsi;
        XXX_v2000h_nvramData.ulCheckSumV4 = nvramData.ulCheckSumV4;

        memcpy(&XXX_v2000h_nvramData.gponSerialNumber[0],&nvramData.gponSerialNumber[0],NVRAM_GPON_SERIAL_NUMBER_LEN);
        memcpy(&XXX_v2000h_nvramData.gponPassword[0],&nvramData.gponPassword[0],NVRAM_GPON_PASSWORD_LEN);
        memcpy(&XXX_v2000h_nvramData.wpsDevicePin[0],&nvramData.wpsDevicePin[0],NVRAM_WPS_DEVICE_PIN_LEN);
        memcpy(&XXX_v2000h_nvramData.wlanParams[0],&nvramData.wlanParams[0],NVRAM_WLAN_PARAMS_LEN);

        XXX_v2000h_nvramData.ulSyslogSize = nvramData.ulSyslogSize;

        memcpy(&XXX_v2000h_nvramData.ulNandPartOfsKb[0], &nvramData.ulNandPartOfsKb[0], sizeof(nvramData.ulNandPartOfsKb));
        memcpy(&XXX_v2000h_nvramData.ulNandPartSizeKb[0], &nvramData.ulNandPartSizeKb[0], sizeof(nvramData.ulNandPartSizeKb));
        memcpy(&XXX_v2000h_nvramData.szVoiceBoardId[0], &nvramData.szVoiceBoardId[0], sizeof(nvramData.szVoiceBoardId));

        memcpy(&XXX_v2000h_nvramData.afeId[0], &nvramData.afeId[0], sizeof(nvramData.afeId));

        memcpy(&XXX_v2000h_nvramData.ulSerialNumber[0],&nvramData.ulSerialNumber[0],32);
        memcpy(&XXX_v2000h_nvramData.chFactoryFWVersion[0],&nvramData.chFactoryFWVersion[0],48);
        memcpy(&XXX_v2000h_nvramData.wpsPin[0],&nvramData.wpsPin[0],32);
        memcpy(&XXX_v2000h_nvramData.wpaKey[0],&nvramData.wpaKey[0],32);
        XXX_v2000h_nvramData.dslDatapump=nvramData.dslDatapump;

        XXX_v2000h_nvramData.ulVersion=NVRAM_VERSION_NUMBER;

        memset((char *)&XXX_v2000h_nvramData + ((size_t) &((NVRAM_DATA *)0)->ulSerialNumber), 0, 1);
        memset((char *)&XXX_v2000h_nvramData + ((size_t) &((NVRAM_DATA *)0)->chFactoryFWVersion), 0, 1);

        XXX_v2000h_nvramData.ulCheckSum = getCrc32((unsigned char *)&XXX_v2000h_nvramData, sizeof(XXX_V2000H_NVRAM_DATA),CRC32_INIT_VALUE);

        writeNvramDataCrcLocked(&XXX_v2000h_nvramData);
        return TRUE;
     }
     return FALSE;
