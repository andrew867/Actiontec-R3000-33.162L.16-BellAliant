...
int AEI_nandImageSet( int partition, char *string, int img_size )
{
    ...
    if( mtd0 && mtd1 )
    {
        ...
        int retlen = 0;
        int blk_addr = 0;
        int total_tag_blks = AEI_TAG_BLOCKS * mtd2->erasesize;
        ...
        if( *(unsigned short *) string != JFFS2_MAGIC_BITMASK &&  *(unsigned short *) string != AEI_MAGIC_BITMASK)
        {
			...
			if(cferombuf != NULL)
			{
                mtd1->_write(mtd1, 0, mtd1->erasesize, &retlen, cferombuf);
                kfree(cferombuf);
			}
			else
                mtd1->_write(mtd1, 0, mtd1->erasesize, &retlen, string);
            string += ofs;
        }
        ...
        {
            for( blk_addr = 0; blk_addr < total_tag_blks; blk_addr += mtd2->erasesize )
               nandEraseBlk( mtd2, blk_addr );
            ...
            else
            {
                ...
                {   
                    printk("##Write tag %8.8x\n", blk_addr);
                    mtd2->_write(mtd2, blk_addr, mtd2->erasesize, &retlen, string);
                }
            }
        ...
        for( blk = fs_start_blk; blk < mtd0->size; blk += mtd0->erasesize )
        {
            ...
            if( (sts = nandEraseBlk( mtd0, blk )) == 0 )
            {
                /* Write a block of the image to flash. */
                if( string < end_string && string != cferam_string )
                {
                    ...
                    mtd0->_write(mtd0, blk, writelen, &retlen, string);
                    if( retlen == writelen )
                    {
                        printk(".");
                        string += writelen;
        ...
        for( blk = 0; blk < fs_start_blk; blk += mtd0->erasesize )
        {
            ...
            if( (sts = nandEraseBlk( mtd0, blk )) == 0 )
            {
                ...
                if( cferam_string )
                {
                    mtd0->_write(mtd0, blk, mtd0->erasesize,
