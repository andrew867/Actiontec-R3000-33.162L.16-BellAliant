...
#if defined(AEI_VDSL_SW_WAN_ETH_LED) || defined(AEI_VDSL_INET_LED_BLINK)
int enetWanLedOn=0;
...
bcm63xx_enet_query(struct net_device * dev)
{
    ...
    if ((port < 0) || (port >= MAX_SWITCH_PORTS)) {
	  ...
	  bcmsw_get_hw_stats(port, extswitch, stats, dev);
#else
	   /* reset stats before collecting - in case the called API does not provide anything */
	  memset(stats,0,sizeof(*stats));
...
#if defined(AEI_63138_CHIP) && defined(CONFIG_BRCM_CHIP_VER_A0)
...
#if defined (CONFIG_BCM_JUMBO_FRAME)
    {
        int we_locked_rtnl = FALSE;
            ...
            if (memcmp(pNetDev->name, "ewan", 4) == 0)
            {
               ...
               enetWanLedOn=0;
#endif
#if defined(AEI_VDSL_INET_LED_BLINK)
...
#if defined(AEI_VDSL_CUSTOMER_NCS) && !defined(AEI_63168_CHIP) && !defined(AEI_63138_CHIP)
...
#if defined(AEI_VDSL_CUSTOMER_NCS) && defined(CONFIG_BCM96368)
    {
        uint16 v16;
