 ...
 *
 * $Id: wlconf.c 417874 2013-08-12 23:24:53Z $
 */
...
#ifdef DSLCPE_WLCONF_MAIN
/* when build wlconf application, we need the API from libarry,so
 * no need to include them in the file*/
int wl_cnt=0;
int wlconf_start(char *name);
int wlconf_security(char *name);
#else
static int
...
#if defined(AEI_VDSL_CUSTOMER_NCS) // Fix compile error for variable set but not used, due to AEI enable Werror for wlan.
	if (ret) ;
#endif
...
wlconf(char *name)
{
...
#ifdef DSLCPE
	static int cur_mbss_on[2]={0,0}; /* for only support wl0 & wl1 */
	int mbss_on=0;
#endif
...
#ifndef __CONFIG_USBAP__
#ifdef DSLCPE
			/* Default set mbss 0 w/ primary BSS only */
			WL_IOVAR_SETINT(name, "mbss", cur_mbss_on[wlunit]);
			if (mbss_on) {
				cur_mbss_on[wlunit] = mbss_on; /* keep the current mbss status */
				WL_IOVAR_SETINT(name, "mbss", cur_mbss_on[wlunit]);
			} /* else, not allow mbss to be set from 1 to 0 */
#else
	...
	*(next = buf) = '\0';
	...
	/*We need to disable vhtmode while the band is 2.4G*/
	if (bandtype == WLC_BAND_2G)
	{
		WL_IOVAR_SETINT(name, "vhtmode", 0);
	}
#endif
