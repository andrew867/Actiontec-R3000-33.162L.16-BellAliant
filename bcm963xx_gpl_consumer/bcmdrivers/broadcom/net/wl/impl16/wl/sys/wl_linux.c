 ...
 *
 * $Id: wl_linux.c 417426 2013-08-09 07:36:58Z $
 */
...
#ifdef WLMEDIA_FLAMES			/* set UDP-RTP checksums to 0 */
#include <linux/ip.h>
#include <linux/udp.h>
#include <linux/in.h>
#endif

#if defined(DSLCPE) && defined(DSLCPE_WLTHREAD_L34)
...
#if (defined(CONFIG_BCM_RDPA)||defined(CONFIG_BCM_RDPA_MODULE)) && defined(CONFIG_BLOG) && defined(DSLCPE)
...
extern void wl_pktc_del(uint32 addr);
extern void (*wl_pktc_del_hook)(uint32_t addr);
#endif
#endif /* DSLCPE */
...
#define TIMER_LOCK(_t) spin_lock_bh(&(_t)->wl_timer_lock)
#define TIMER_UNLOCK(_t) spin_unlock_bh(&(_t)->wl_timer_lock)
#endif
...
#endif
#if defined(PKTC) && defined(CONFIG_BCM_WFD_CHAIN_SUPPORT) && defined(CONFIG_BCM96838)
...
#ifdef DSLCPE
static int omitdonglehw = 0;
module_param(omitdonglehw, int, 0);
#endif
...
/* Broadcom extension for legacy/ht variant of the header. This header
 * is also used to indicate VHT b/w signalling information for
 * legacy PHY - namespace, MCS and VHT fields
 */
...
static void AEI_MultiMirrorPacket( struct sk_buff *skb, uint16 mirFlags, bool mirrorIn )
{
    int i;
    uint32 flag;
...
wl_worker_thread_func(void *data)
{
	...
	/* don't need to set PF_NOFREEZE since DSLCPE systems do not suspend */

	while (1)
	{
		wait_event_interruptible(wl->thread_wqh,
		                            (wl->txq_dispatched ||
#ifdef WLC_LOW
		                             wl->rxq_dispatched ||
#endif
...
#ifdef DSLCPE_KTHREAD_TIMER
static int
wl_worker_tmr_thread_func(void *data)
{
	wl_info_t *wl = (wl_info_t *) data;

	while (1)
	{
		wait_event_interruptible(wl->tmr_thread_wqh,
		                             (wl->wl_timer_dispatched_any ||
		                             kthread_should_stop()));
		if (kthread_should_stop())
		{
			printk(KERN_INFO "kthread_should_stop detected on wl%d\n", wl->unit);
		...
		if (wl->wl_timer_dispatched_any) {
			struct wl_timer *t;
			for (t = wl->timers; t; t = t->next) {
				if (t->wl_timer_dispatched)
					wl_timer_task(&t->wl_timer_task);
			}
		}
...
#endif /* DSLCPE_KTHREAD_TIMER */

#endif /* WL_ALL_PASSIVE */
#endif /* DSLCPE and Linux 3.4.0 or greater */
...
{
	wl_pktc_req(DUMP_BRC_HOT, 0, 0, 0);
	return 0;
}
#endif
	...
	uint bustype, void *btparam, uint irq, uchar* bar1_addr, uint32 bar1_size)
{
	...
	//This is a workaround for the deadlock issue. In 63168 chip, FAP is enabled. it is not reproduced easily.In 6368 chip, it is reproduced easily. So bind the wireles to CPU 0 temporarily. Bo is trying the new solution.
				wl->processor_id = 1;
#else
				wl->processor_id = 0;
#endif
...
#if defined(DSLCPE) && defined(DSLCPE_WLTHREAD_L34)
	{
		...
		init_waitqueue_head(&wl->thread_wqh);
		sprintf(kthrd_namebuf, "wl%d-kthrd", unit);
		printk(KERN_INFO "wl%d: creating kthread %s\n", unit, kthrd_namebuf);
		wl->thread = kthread_create(wl_worker_thread_func, wl, kthrd_namebuf);
		if (IS_ERR(wl->thread)) {
			printk(KERN_ERR "wl%d: failed to create kernel thread\n", unit);
			goto fail;
		}
		wake_up_process(wl->thread);
		if (call_usermodehelper_taskset((int)wl->thread->pid, wl->processor_id)) {
			printk(KERN_ERR "wl%d: could not bind kernel thread to CPU\n", unit);
			goto fail;
		}
	}
#ifdef DSLCPE_KTHREAD_TIMER
	{
		char kthrd_namebuf[32]={0};
		init_waitqueue_head(&wl->tmr_thread_wqh);
		sprintf(kthrd_namebuf, "wl%d-tmr-kthrd", unit);
		printk(KERN_INFO "wl%d: creating timer kthread %s\n", unit, kthrd_namebuf);
		wl->tmr_thread = kthread_create(wl_worker_tmr_thread_func, wl, kthrd_namebuf);
		if (IS_ERR(wl->tmr_thread)) {
			printk(KERN_ERR "wl%d: failed to create kernel thread for wl timer\n", unit);
			goto fail;
		}
		wake_up_process(wl->tmr_thread);
		if (call_usermodehelper_taskset((int)wl->tmr_thread->pid, wl->tmr_tp_id)) {
			printk(KERN_ERR "wl%d: could not bind kernel thread to CPU\n", unit);
			goto fail;
		}
	}
#endif
#endif /* DSLCPE and Linux 3.4.0 or later */
#endif /* WL_ALL_PASSIVE */
#if defined(DSLCPE) && defined(NAPI_POLL)
...
#ifdef DSLCPE_KTHREAD_TIMER
	wl->wl_timer_dispatched_any = FALSE;
#endif
	...
	if (kerSysGetWlanFeature()&WLAN_FEATURE_DHD_NIC_ENABLE) 
		printk("--- NIC MODE is enabled --- \r\n");
	else
		printk("--- DHD MODE is enabled \r\n");
#endif
	/* Populate wlcif of the primary interface in wlif */
	wlif->wlcif = wlc_wlcif_get_by_index(wl->wlc, 0);

#ifdef DSLCPE_PREALLOC_SKB
	/* moved this part code from top to here after wlc_attach to hw to get correct chipid */
	...
	if (allocskbmode)
	{
		...
		else
		{
			...
			if (i.totalram <= MEMSZ_32MB/i.mem_unit) {
				if (isac) 
					currallocskbsz = 512; /* when the chip is AC, it need more than 320 permenant allocation */
				else 
					currallocskbsz = 256;
	...
	if ((device_speed != FULL_SPEED) && (device_speed != LOW_SPEED)) {
		...
		wlc_iovar_setint(wl->wlc, "ampdu_mpdu",
			wl->pub->ampdu_mpdu);
#ifdef WLMEDIA_TXBF_ENB
		wlc_iovar_setint(wl->wlc, "txbf_bfr_cap", 1);
		wlc_iovar_setint(wl->wlc, "txbf_bfe_cap", 1);
		wlc_iovar_setint(wl->wlc, "txbf", 1);
#endif
...
#if (defined(CONFIG_BCM_RDPA)||defined(CONFIG_BCM_RDPA_MODULE))
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
    if ((wfd_idx = wfd_bind(dev, WFD_WL_FWD_HOOKTYPE_SKB, true, wl_bulk_txchainhandler, 0)) != -1)
#else
    if ((wfd_idx = wfd_bind(dev, WFD_WL_FWD_HOOKTYPE_SKB, true, wl_txchainhandler, 
                            wl_txchainhandler_complete)) != -1)
#endif
    {
        if (wl_pktc_req(UPDATE_WFD_IDX_BY_WL_DEV, (uint32_t)dev, (uint32_t)wfd_idx, 0) != 0)
        {
            WL_ERROR(("wl%d: Unable to update WFD handle entry for dev 0x%p wfd_idx 0x%x\n", unit, dev, wfd_idx));
        }
    }
    else
    {
        WL_ERROR(("wl%d: Error in binding WFD\n", unit));
    }
#else
...
#if defined(DSLCPE_PKTC)
        wl->pub->brc_hot = (ctf_brc_hot_t *)wl_pktc_req(BRC_HOT_GET_TABLE_TOP, 0, 0, 0);
#endif
#ifdef HNDCTF
...
wl_alloc_linux_if(wl_if_t *wlif)
{
    ...
    if (wl->unit == 1)
    {
        if (wlif->wlcif == NULL)
        { 
           subunit =  WL_NUM_OF_SSID_PER_UNIT;
        }
        else
        {
           subunit = subunit + WL_NUM_OF_SSID_PER_UNIT;
        }
    }
	...
	switch (cmd) {
		...
		case SIOCPORTMIRROR:
		{
			...
			else
			{
				...
				{
					AEI_UpdateMirrorFlag(&(wlif->usMirrorInFlags),
						  mirrorCfg.szMirrorInterface, mirrorCfg.nStatus);
				}
				else /* MIRROR_DIR_OUT */
				{
					AEI_UpdateMirrorFlag(&(wlif->usMirrorOutFlags),
						  mirrorCfg.szMirrorInterface, mirrorCfg.nStatus);
				}
			}
			return 0;
	...
	if ((ours = wlc_isr(wl->wlc, &wantdpc))) {
		...
		if (wantdpc) {
			...
			if (WL_ALL_PASSIVE_ENAB(wl)) {
				...
				if (1)   /* looks weird, but preserves structure of code */
#else
...
wl_dpc(ulong data)
{
	...
	if (wl->resched) {
		if (!(WL_ALL_PASSIVE_ENAB(wl)))
			 ...
			 */
			if (0)      /* looks weird, but preserves structure of code */
#else
			if ((brcm_smp)?(!schedule_work_on(wl->processor_id, &wl->wl_dpc_task.work)):(!schedule_work(&wl->wl_dpc_task.work)))
#endif
#else
...
#endif /* NAPI_POLL */
{
	...
	WL_TRACE(("wl%d: wl_poll: rxbnd %d quota %d budget %d processed %d\n",
	          wl->pub->unit, wl->pub->tunables->rxbnd, dev->quota,
	          *budget, wl->processed));
...
wl_sendup(wl_info_t *wl, wl_if_t *wlif, void *p, int numpkt)
{
	...
	if (wlif)
	{
		...
		{
			AEI_MultiMirrorPacket( tmp_skb , wlif->usMirrorInFlags, TRUE );
		}
	}
	else
	{
		if (main_wlif && main_wlif->usMirrorInFlags != 0 &&
		    eth->h_proto != ETHER_TYPE_BRCM)
		{
			AEI_MultiMirrorPacket( tmp_skb , main_wlif->usMirrorInFlags, TRUE );
		}
	}
#endif /* AEI_VDSL_TOOLBOX */
...
#endif
#if defined(CONFIG_BLOG) && defined(DSLCPE)
        /* If Linux TCP/IP stack is bypassed by injecting this packet directly to fastlath, then avoid software FC - it
         * will probably be slower than fastpath. */
	if (wl->pub->fcache
#if defined(PKTC) && defined(CONFIG_BCM_WFD_CHAIN_SUPPORT) && defined(CONFIG_BCM96838)
			&&( !inject_to_fastpath || PKTISCHAINED(skb))
...
#ifdef WLMEDIA_FLAMES
	eh = eth_hdr(skb);
	if (ntoh16(eh->h_proto) == ETH_P_IP) {
		struct iphdr *iphdr = (struct iphdr *) ((char *) eh + sizeof(*eh));
		struct udphdr *udphdr = (struct udphdr *)((char *)iphdr + sizeof(*iphdr));
		if (iphdr->protocol == 0x11) {
			/* Set UDP checksum to 0, avoid checksum computation in IP stack */
			udphdr->check = 0;
		}
	}
#endif
...
wl_start(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (IS_SKBUFF_PTR(skb))
#endif /* CONFIG_BLOG && DSLCPE */
		memset(&skb->cb[0], 0, sizeof(skb->cb));
#endif
...
#ifdef DSLCPE_CACHE_SMARTFLUSH
{
	...
	{
		AEI_MultiMirrorPacket( skb, wlif->usMirrorOutFlags, FALSE );
	}
#endif
...
#if defined(DSLCPE_WLTHREAD_L34)
		/*
		 * In Linux 3.4, we have a dedicated wl processing thread.  So as
		 * long as wl->txq_dispatched remains TRUE, we do not need to
		 * explicitly re-schedule it again.  It just keeps running.
		 */
#else
...
#if defined(DSLCPE_KTHREAD_TIMER) && !defined(DSLCPE_WLTHREAD_L34)
	if ((brcm_smp)?(!schedule_work_on(t->wl->tmr_tp_id, &t->wl->wl_timer_task.work)):(!schedule_work(&t->wl->wl_timer_task.work))) {
		WL_ERROR(("wl%d: wl_timer_task/schedule_work failed\n", t->wl->pub->unit));
	}
#endif
...
wl_timer(ulong data)
{
    ...
    {
		TIMER_LOCK(t);
		if (t->wl_timer_dispatched == FALSE) {
			t->wl_timer_dispatched = TRUE;
			t->wl->wl_timer_dispatched_any = TRUE;
#if defined(DSLCPE_WLTHREAD_L34)
			wake_up_interruptible(&t->wl->tmr_thread_wqh);
#endif
		}
		TIMER_UNLOCK(t);
    }
...
#ifdef DSLCPE_KTHREAD_TIMER
#if !defined(DSLCPE_WLTHREAD_L34)
	MY_INIT_WORK(&t->wl_timer_task.work, (work_func_t)wl_timer_task);
#endif
	t->wl_timer_task.context = t;
	t->wl_timer_dispatched = FALSE;
	spin_lock_init(&t->wl_timer_lock);
#endif
...
#ifdef DSLCPE
					stats->multicast =WLCNTVAL(wlcif_stats.rxmulti);
					stats->tx_multicast_packets =WLCNTVAL(wlcif_stats.txmulti);
...
#if defined(DSLCPE_WLTHREAD_L34)
	if ( processor_id == 0 ||
	     (brcm_smp && processor_id == 1) ||
	     processor_id == -1 ) {
		pid_t pid = ((wl_info_t *)wl)->thread->pid;
...
#else
	if ( processor_id == 0 || (brcm_smp && processor_id == 1) ) {
#endif
...
#ifdef DSLCPE_KTHREAD_TIMER
int wl_get_tmr_tp_id(void *wl)
{
	return ((wl_info_t *)wl)->tmr_tp_id;
}

void wl_set_tmr_tp_id(void *wl, int processor_id)
{
#if defined(DSLCPE_WLTHREAD_L34)
	if ( processor_id == 0 ||
	     (brcm_smp && processor_id == 1) ||
	     processor_id == -1 ) {
		pid_t pid = ((wl_info_t *)wl)->tmr_thread->pid;
		if (call_usermodehelper_taskset((int) pid, processor_id) != 0) {
			printk(KERN_ERR "usermodehelper_taskset failed!  "
			                "Unable to set wlan kthread pid %d to CPU%d\n",
			                (int) pid, processor_id);
	...
	if ( processor_id == 0 || (brcm_smp && processor_id == 1) ) {
#endif
		((wl_info_t *)wl)->tmr_tp_id = processor_id;
	}
	else
		wl_set_processor_id_usage();
...
bool wl_pkt_drop_on_wmark(void *wl_ptr, struct sk_buff *skb, bool is_pktc)
{
	wl_info_t *wl = (wl_ptr == NULL) ? WL_INFO(skb->dev) : (wl_info_t *)wl_ptr;
	
	return wl_pkt_drop_on_wmark_check(wl->unit, is_pktc);
}
...
int wl_check_fdb_expired(unsigned char *addr)
{
	...
	brc_hot = (ctf_brc_hot_t *)wl_pktc_req(BRC_HOT_GET_BY_DA, (uint32_t)addr, 0, 0);
...
void wl_set_txq_dispatched(wl_pktc_info_t *wl_pktci, bool flag) {
    wl_if_t *wlif = (wl_if_t *)wl_pktci->wlif;
    wl_info_t *wl = wlif->wl;
...
bool wl_get_txq_dispatched(wl_pktc_info_t *wl_pktci) {
    wl_if_t *wlif = (wl_if_t *)wl_pktci->wlif;
...
int wl_pktc_init(wl_if_t *wlif, struct net_device *dev) {
    ...
    wl_pktc_req(UPDATE_WLAN_HANDLE, (uint32_t)wl_pktci, (uint32_t)dev, 0);
...
void wl_free_pktc(wl_if_t *wlif) {
    ...
    if (wlif->pktci) {
        wl_pktc_req(DELETE_WLAN_HANDLE, (uint32_t)wlif->pktci, 0, 0);
