...
void br_dhcp_option(struct udphdr * uh, struct sk_buff *skb)
{
       ...
       if (skb->dev && (skb->dev->priv_flags & IFF_WANDEV))     //only process LAN Side dhcp packet.
           return;

       memset(tmp_vendorid, 0, sizeof(tmp_vendorid));
       udlen = ntohs(uh->len);
       cpylen = sizeof(packet) < udlen?sizeof(packet):udlen;
       memcpy(&packet, uh, cpylen);
