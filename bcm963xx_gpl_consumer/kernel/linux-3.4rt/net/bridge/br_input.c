...
#if 0
/*In 1601, there is no pip struct, and sync to richard*/
        ...
        if ((pip) && (pip->protocol == 17))
        {
                ...
                if((ntohs(uhead.source) == 68)&&(ntohs(uhead.dest) == 67)&&strcmp(br->dev->name, "br1")==0)
                {
                        /*It's a dhcp packet*/
                        uh = (struct udphdr *)((unsigned char *)pip + iphead_len);
                        br_dhcp_option(uh, skb);
                }
        }
#endif
#endif
