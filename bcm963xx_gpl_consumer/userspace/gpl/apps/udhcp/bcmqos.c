...
{
    struct optioncmd *pnode = optioncmdHead;
    char vendorString[VENDOR_CLASS_ID_STR_SIZE + 1] = { 0 };
    char tempcmd[256] = { 0 };
		   ...
		   for (i = 0; i < iface->max_leases; i++)
		   {
            ...
            if (lease_expired(&(iface->leases[i])))
			   ...
			   switch (pnode->optionnum)
			   {
				   case DHCP_VENDOR:
#if !defined(AEI_VDSL_QOS)
                    //For XXX project, we will apply ebtables rules using the mac address of the LAN device, rather than IP address,
