...
static char **fill_envp(struct dhcpMessage *packet)
{
        ...
        sprintf(dhcpc_opt.ip, "%d.%d.%d.%d",addr[0], addr[1], addr[2], addr[3]);
       ...
       AEI_sendEventMessageWith6RD(TRUE,FALSE,&dhcpc_opt);
#endif
#if defined(AEI_VDSL_CUSTOMER_DHCP_WAN_OPTION121)
...
void run_script(struct dhcpMessage *packet, const char *name)
{
   ...
   if (!strcmp(name, "bound")||!strcmp(name, "renew"))
...
#endif
	{
     ...
     {
         memset(&dhcpc_opt,0,sizeof(&dhcpc_opt));
         strcpy(dhcpc_opt.ip,"1");
         strcpy(dhcpc_opt.mask,"1");
         strcpy(dhcpc_opt.gateway,"1");
         strcpy(dhcpc_opt.nameserver,"1");
         AEI_sendEventMessage(FALSE,TRUE , &dhcpc_opt);
