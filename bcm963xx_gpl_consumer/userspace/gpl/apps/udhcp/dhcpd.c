		...
		else
		{
			lease = find_lease_by_chaddr(packet.chaddr);
                        ...
                        if(lease && state[0] == DHCPREQUEST)
                        {
                            ...
                            if (!(ntohl(lease->yiaddr) >= ntohl(cur_iface->start) && ntohl(lease->yiaddr) <= ntohl(cur_iface->end)))                     {
                                sendNAK(&packet); 
                                continue;
                             }
#endif
					...
					if (hostname) {
						...
						if (strstr(lease->hostname, "\(") ||
							...
							if (!strncmp(lease->hostname, "SipuraSPA", bytes)) {
								printf("Linksys VoIP ATA device!\n");
								iconflag = 7;
...
#else
							strncpy(lease->hostname, "Video", sizeof(lease->hostname));
#endif
						}
#endif
...
#ifdef SUPPORT_GPL_UNDEFINED //add william 2012-4-25
