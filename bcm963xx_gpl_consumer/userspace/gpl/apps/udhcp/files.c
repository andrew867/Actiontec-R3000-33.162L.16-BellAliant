...
#endif
#if defined(AEI_VDSL_CUSTOMER_DHCP_OPTION60)
     for (i = 0; i < VENDOR_CLASS_ID_TAB_SIZE; i++) {
...
#if defined(SUPPORT_GPL_UNDEFINED) || defined(SUPPORT_GPL)   //hk_XXX//hk_ctl
...
void read_leases(char *file)
{
    ...
    while ((fread(&lease, sizeof lease, 1, fp) == 1)) {
        for (iface = iface_config; iface; iface = iface->next) {
#if defined(SUPPORT_GPL)  /*need host byte order*/
          if (ntohl(lease.yiaddr) >= ntohl(iface->start) && ntohl(lease.yiaddr) <= ntohl(iface->end) && iface->cnt_leases < iface->max_leases) {          
#else
