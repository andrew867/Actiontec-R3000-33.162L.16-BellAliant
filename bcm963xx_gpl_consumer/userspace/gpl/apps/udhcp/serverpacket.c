...
#endif
#if defined(AEI_VDSL_CUSTOMER_DHCP_OPTION66)|| defined(AEI_VDSL_CUSTOMER_DHCP_OPTION120)
...
/* This function is written for XXX DHCP option 67 requirement
...
#if defined(AEI_VDSL_DHCP_LEASE)||defined(SUPPORT_GPL_UNDEFINED)
...
int sendOffer(struct dhcpMessage *oldpacket)
{
    ...
    if (!static_lease_ip) {
        ...
        if (lease && !reservedIp(cur_iface->static_leases, lease->yiaddr) && ntohl(lease->yiaddr) >= ntohl(cur_iface->start) && ntohl(lease->yiaddr) <= ntohl(cur_iface->end))
#else
...
#ifndef AEI_VDSL_DHCP_AUTO_RESERVATION
...
#endif
#endif
#if defined(SUPPORT_GPL_UNDEFINED) || defined(AEI_VDSL_DHCP_AUTO_RESERVATION)
                if (!packet.yiaddr)
...
#if defined(SUPPORT_GPL_UNDEFINED) && defined(AEI_VDSL_DHCP_LEASE)
...
#endif
#ifdef AEI_VDSL_CUSTOMER_DHCP_OPTION66
...
#endif
#ifdef AEI_VDSL_CUSTOMER_DHCP_OPTION67
          if (curr->data[OPT_CODE] == DHCP_BOOT_FILE) 
          {
               char *verdid = NULL;
               
               verdid = get_option(oldpacket, DHCP_VENDOR);
               if(verdid != NULL)
                add_option_string(packet.options, curr->data); 
              curr = curr->next;
              continue;
...
#endif
#ifdef AEI_VDSL_CUSTOMER_DHCP_OPTION120
...
#endif
#if defined(SUPPORT_GPL_UNDEFINED) //add william 2012-4-25
