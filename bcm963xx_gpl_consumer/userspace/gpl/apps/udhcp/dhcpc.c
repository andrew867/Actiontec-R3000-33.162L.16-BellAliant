...
void AEI_sendEventMessageWith6RD(UBOOL8 assigned, UBOOL8 isExpired, DhcpcStateChangedMsgBody *options)
{
...
#if defined(AEI_VDSL_CUSTOMER_NCS)
	   /*
	    * The original code looks strange here.
	    * The logic will goes into here When state is RELEASED and then
	    * set listen_mode and state etc.
	    * However, line 483 will never goes into when state is RELEASED.
	    * I do not understand why the original author write the code like this.
	    * It is conflict when state is RELEASED, listen_mode want to set LISTEN_KERNEL or LISTEN_RAW ?
	    *
	    * Why fix the bug which do release then do renew does not work like this:
	    * rootcause: Here set listen_mode to LISTEN_KERNEL(dhcpc will use non-raw(layer 3) socket to receive packet),
	    *            it will caused dhcpc can not get ACK packet. but the driver already got the ACK packet actually. The packet filter out by kernel ?
	    *            So dhcpc will sent request packets again and again.
	    * fix: 1, Change listen_mode to LISTEN_RAW, use raw socket(layer 2) to receive packet works fine.
	    *      2, The original code in line 483 also want to use LISTEN_RAW when state is RELEASED(but dhcpc never goes there).
	    *      3, When dhcpc successfully do the 1st dhcp procedure, discover --> offer --> request --> ACK,
	    *         the listen_mode is always used LISTEN_RAW.
	    *
	    * And I do not think server_addr should be set to 0 here in our case(only state == BOUND || state == RELEASED),
	    * otherwise dhcpc will send broadcast request packet.
	    */
	    if (state == RELEASED)
...
/*
  ...
  #if defined(SUPPORT_GPL_UNDEFINED) //add william 2012-3-15
